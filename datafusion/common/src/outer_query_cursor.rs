    ...
    pub fn field_value(&self, name: &str) -> Result<ScalarValue> {
        ...
        let (batch, pos) = guard.as_ref().ok_or_else(|| {
            DataFusionError::Execution(format!(
                "Trying to get '{name}' column on uninitialized OuterQueryCursor",
            ))
        })?;
...
impl PartialEq for OuterQueryCursor {
    fn eq(&self, other: &Self) -> bool {
        self.schema == other.schema
            && *self.record_batch_and_pos.read().unwrap()
                == *other.record_batch_and_pos.read().unwrap()
    }
}
