...
#[async_trait]
impl ExecutionPlan for CubeSubqueryExec {
    ...
    fn with_new_children(
        ...
        if children.len() < 2 {
            return Err(DataFusionError::Internal(format!(
                "CubeSubqueryExec should have not less than 2 children but got {}",
                children.len()
            )));
        }

        Ok(Arc::new(CubeSubqueryExec::try_new(
                        ...
                        for (subquery_i, subquery) in subqueries.iter().enumerate() {
                            let null_array = || {
                                ...
                                if fields.len() != 1 {
                                    ...
                                    "Cube sub query should have only one column but got {}",
                                    fields.len()
                                )));
                                }
                            ...
                            if subquery.output_partitioning().partition_count() != 1 {
                                return Err(ArrowError::ComputeError(format!("Cube sub query should have only one partition but got {}", subquery.output_partitioning().partition_count())))
                            }
                            let mut stream = subquery.execute(0, context.clone())?;
                            ...
                            if let Some(subquery_batch) = res {
                                ...
                                match subquery_batch.column(0).len() {
                                    ...
                                    _ => return Err(ArrowError::ComputeError(
                                        "Cube sub query should return no more than one row"
                                            .to_string(),
                                    )),
                                };
                            } else {
...
/// CubeSubquery iterator
struct CubeSubqueryStream {
    stream: Pin<Box<dyn Stream<Item = ArrowResult<RecordBatch>> + Send>>,
    schema: SchemaRef,
    size_hint: (usize, Option<usize>),
}

impl Stream for CubeSubqueryStream {
...
impl RecordBatchStream for CubeSubqueryStream {
    /// Get the schema
    fn schema(&self) -> SchemaRef {
        self.schema.clone()
    }
