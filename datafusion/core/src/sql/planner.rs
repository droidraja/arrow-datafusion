...
impl<'a, S: ContextProvider> SqlToRel<'a, S> {
    ...
    fn extract_udtf(&self, exprs: Vec<Expr>) -> Vec<Expr> {
        ...
        for expr in exprs.iter() {
            ...
            if let Expr::Alias(a_expr, ..) = expr {
                udtfs.extend(self.extract_udtf([a_expr.as_ref().clone()].to_vec()).iter().map(|e| e.clone()));
            }
        }
    ...
    fn rewrite_exprs_udtf_node(&self, exprs: Vec<Expr>, schema: DFSchemaRef) -> Vec<Expr> {
        let mut rewrote_exprs = Vec::new();
        for expr in exprs.iter() {
            if let Expr::Alias(a_expr, ..) = expr {
                rewrote_exprs.extend(self.rewrite_exprs_udtf_node([a_expr.as_ref().clone()].to_vec(), schema.clone()).iter().map(|e| e.clone()));
            } else if let Expr::BinaryExpr { left, right, .. } = expr {
                let b_exprs: Vec<Expr> = [*left.clone(), *right.clone()].to_vec();
                rewrote_exprs.extend(self.rewrite_exprs_udtf_node(b_exprs, schema.clone()).iter().map(|e| e.clone()));
            } else {
    ...
    // TODO: Binary 
    fn replace_udtf_to_columns(&self, exprs: Vec<Expr>, schema: DFSchemaRef) -> Vec<Expr> {
        let mut rewrote_exprs = Vec::new();
        for expr in exprs.iter() {
            if let Expr::TableUDF { .. } = expr {
                let column = datafusion_expr::Expr::Column(Column { relation: None, name: expr.name(&schema).unwrap() });
                rewrote_exprs.push(column);
            } else if let Expr::BinaryExpr { left, op, right } = expr { 
                let left: Expr = self.replace_udtf_to_columns([left.as_ref().clone()].to_vec(), schema.clone())[0].clone();
                let right: Expr = self.replace_udtf_to_columns([right.as_ref().clone()].to_vec(), schema.clone())[0].clone();
                rewrote_exprs.push(Expr::BinaryExpr { left: Box::new(left), op: *op, right: Box::new(right) });
            } else if let Expr::Alias(a_expr, name) = expr {
                let a_expr: Expr = self.replace_udtf_to_columns([a_expr.as_ref().clone()].to_vec(), schema.clone())[0].clone();
